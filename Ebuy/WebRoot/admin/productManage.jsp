...
<script type="text/javascript">
	...
	function setProductHot(){
		var selectedRows=$("#dg").datagrid('getSelections');
		if(selectedRows.length==0){
			$.messager.alert("系统提示","请选择要设置热卖的数据！");
			return;
		}
		var strIds=[];
		for(var i=0;i<selectedRows.length;i++){
	...
	function setProductSpecialPrice(){
		var selectedRows=$("#dg").datagrid('getSelections');
		if(selectedRows.length==0){
			$.messager.alert("系统提示","请选择要设置特价的数据！");
			return;
		}
		var strIds=[];
		for(var i=0;i<selectedRows.length;i++){
...
<body style="margin:1px;">
	<table id="dg" title="商品管理" class="easyui-datagrid" fitColumns="true" pagination="true" rownumbers="true"
		...
		<thead>
			<tr>
				...
				<th field="specialPriceTime" width="50" align="center" hidden="true">特价时间</th>
			</tr>
		</thead>
	</table>
	...
	<div id="dlg" class="easyui-dialog" style="width:600px;height:500px;padding:10px 20px;" closed="true" buttons="#dlg-buttons">
		<form id="fm" action="" method="post" enctype="multipart/form-data">
			<table style="border-collapse:separate; border-spacing:5px 15px;">
				<tr>
					<td>商品名称：</td>
					<td colspan="4"><input type="text" id="name" name="product.name" class="easyui-validatebox" required="true" width="300px"></td>
				</tr>
				<tr>
					<td>价格：</td>
				...
				<tr>
					<td>所属大类：</td>
					<td colspan="4"><input id="bName" name="product.bigType.id" class="easyui-combobox" data-options="panelHeight:'auto',editable:false,valueField:'id',textField:'name',url:'productBigType_comboList.action'"></td>
				</tr>
				<tr>
					<td>所属小类：</td>
					<td colspan="4"><input id="sName" name="product.smallType.id" class="easyui-combobox" data-options="panelHeight:'auto',editable:false,valueField:'id',textField:'name',url:'productSmallType_comboList.action'"></td>
				</tr>
				<tr>
					<td valign="top">备注：</td>
					<td colspan="4">
						<textarea rows="7" cols="50" id="description" name="product.description"></textarea>
	...
	<div id="dlg-buttons">
		...
		<a href="javascript:closeProductAddDialog()" class="easyui-linkbutton" iconCls="icon-cancel">关闭</a>
	</div>
</body>
</html>